...
# pillow library used
# OpenCV library used
# ImagicMagick (wand) library used
...
for fname in glob.glob(filepath + "/" + "*.tif"):
	...
	bw_conv.save(output + "del" + name, dpi=(dpi_val,dpi_val), compression=comp_img)
	name1 = output + "del" + name

	# Pillow converted output is lower version tiff image, so used the ImageMagicK library and converted into TIFF version 5.0
	with wand.image.Image(filename=name1) as img:
		img.save(filename=output + name)
		os.remove(output + "del" + name)
